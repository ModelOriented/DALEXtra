...
The `DALEXtra` package is an extension pack for [DALEX](https://modeloriented.github.io/DALEX) package. It contains various tool for XAI (eXplainable Artificial Intelligence) that can help us inspect and improve our model. Functionalities of the `DALXtra` could be divided into three areas.

* Champion-Challenger analysys
 + Let us compare two or more Machine-Learning models and try to determinate which one is better or improve both of them
...
* Cross langauge comaprison
 + Creating explainers for models created in different languges so they can be explained using R tools like dr.why.ai family
 + Currently supported are Python scikit-learn and keras, Java h2o and mljar, R mlr and mlr3
* Aspect Importance analysys
...
Without aby doubt, comaprison of models, espacially black-box ones is very important use case nowadays. Every day new models are being created and we need to tools that can allow us to determinate which one is better. For this purpose we present Champion-Challenger analysys. It is set of functions that creates comaprisons of models and later can be gethered up to created one report with generic comments. Example of report can be found ![here](). As you can see any explenation that has generic `plot()` function can be plotted. 

## Funnell Plot

Core of our analysys is funnel plot.
